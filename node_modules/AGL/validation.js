...
var validate = require('validate.js'); // TODO Implement this to do all validation below
...
function ValidateAGLUsername(value) {
	...
	if (value.length > 25) {
		return new TypeError('Username: ' + value + ' is invalid becasue it exceeds 25 characters');
	}

	var forbiddenChars = /\W/;
	if (forbiddenChars.test(value)) {
		return new TypeError('Username: ' + value + ' is invalid because it contains a non-word character.'
...
function ValidateAGLPasswordHash(value) {
	...
	if (value.length !== 16) {
		return new TypeError('Password must be a hash of length 16 characters.');
	}
...
function ValidateEmail(value) {
	...
	if (value.length > 50) {
		return new TypeError('Email: ' + value + ' must not exceed 50 characters.');
	}
	...
	if (!RFC822ValidEmail.test(value)) {
		return new TypeError('Email: ' + value + ' is not a valid email address.');
	}
...
function ValidateAGLPublicId(value) {
	...
	if (value.length !== 8) {
		return new TypeError('Public ID: ' + value + ' must be 8 characters long.');
	}

	var forbiddenChars = /[\W_]/;
	if (forbiddenChars.test(value)) {
		return new TypeError('Public ID: ' + value + ' is invalid. Public Ids must only contain digits or letters.');
	}
