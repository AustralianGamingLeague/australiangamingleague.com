...
var ENFORCE_FUNCTIONS_BY_DEFAULT = true;
...
module.exports.NewMockObject = function (objectClass, enforceAllFunctions) {
	if (!validate.isDefined(objectClass)) {
		objectClass = DEFAULT_CLASS;
	}
	if (!validate.isDefined(enforceAllFunctions)) {
		enforceAllFunctions = ENFORCE_FUNCTIONS_BY_DEFAULT;
	}
	var mockObject = new MockObject();
	return Proxy.create(handlerMaker(mockObject, enforceAllFunctions));
}
...
/*  private handlerMaker(obj, enforceAllFunctions)
 *
 *  Handler maker.
 *
 *  Implements all traps for proxy.
 *  All functions pass through to the original object except 'get'
 *  'get' is overridden to check for undefined functions and depending on
 *  the enforceAllFunctions flag will return an error or just ignore it.
 *
 *  FIXME This is ugly. Need to replace this as soon as the new Proxy API
 *  is available in Node.
 */
function handlerMaker(obj, enforceAllFunctions) {
  return {
    // Fundamental traps
    ...
    get:          function(receiver, name) {
					if (obj[name] === undefined) {
						if (enforceAllFunctions) {
							throw new Error('Cannot call unmocked function ' + name
