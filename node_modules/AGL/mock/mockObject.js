 ...
 *      Will default to true. If true an error will be thrown if a function 
 *      which is not mocked is called on the mock object. This should only be
 *      set to false if you are mocking an object that supports chaining and
 *      you only care about certain functions being called.
 *
...
function MockObject (objectClass, enforceAllFunctions) {
	...
	// override get
	// Throw an error if an unmocked function is called.
	this.get = function (target, property) {
		if (target[property] === 'undefined') {
			if (enforceAllFunctions) {
				throw new Error('unmocked function ' + property + '()');
			}
			else {
				return self;
			}
