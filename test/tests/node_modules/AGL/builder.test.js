...
describe('builder', function () {
	...
	var validationMock = {
		Validate : function () {
			return testData.validation.result;
		}
	};
	...
	describe('Builder Object', function () {
		...
		it('should call validation error handler with an array of errors when validation fails', function () {
			...
			testData.validation.result = validationError;
		...
		it('should throw an error if Build() is called and required'
			...
			assert.throw(
				function () {
					testObjectBuilder.OnValidationErrors(function () {}).Build();
				},
				/Property \'prop1\' is required to create a new testObject object./,
				'expected to throw an error'
			);
		});

		it('should set relevant properties to their defaults only if they were unset', function () {
			var propertyMap = {
				prop1 : {
					required      : false,
					type          : 'none',
					default_value : 'prop1 default'
				},
				prop2 : {
					required      : false,
					type          : 'none',
					default_value : 'prop2 default'
				}
			};
			var testObjectBuilder = builder.NewBuilder(propertyMap, 'testObject');
			var testObject = testObjectBuilder
			...
			assert.strictEqual(
				testObject.GetProp1(),
				'prop1 default',
				'expected \'prop1\' to be set to its default'
			);
			assert.strictEqual(
				testObject.GetProp2(),
				'property2',
				'expected \'prop2\' to set to what it was set to, not its default'
			);
		});

		it('should set properties with database defaults to'
				+ ' undefined only if they were unset', function () {
			var propertyMap = {
				prop1 : {
					required         : false,
					type             : 'none',
			...
			assert.strictEqual(
				testObject.GetProp1(),
				undefined,
				'expected \'prop1\' to be undefined'
			);
			assert.strictEqual(
				testObject.GetProp2(),
				'property2',
				'expected \'prop2\' to be what it was set to, not undefined'
			);
		});
		...
		it('should reset after building an object', function () {
			...
			assert.throw(
				function () {
					var testObject2 = testObjectBuilder.Build();
				},
				/Call OnValidationErrors\(handler\) before calling Build\(\)/,
				'expected to throw an error becasue OnValidationErrors() was not called'
			);
			assert.throw(
				function () {
					var testObject3 = testObjectBuilder.OnValidationErrors(function () {}).Build();
				},
				/Property \'prop1\' is required to create a new testObject object/,
				'expected to throw an error because required properties were not set'
			);
		});
